...
class WeatherClient:
    ...
    def find_stations(
        ...
        available_stations = all_stations[
            (all_stations['daily_start'] <= start_dt) & 
            (all_stations['daily_end'] >= end_dt)
        ]
    ...
    def download_weather_data(
        ...
        if 'weather_condition' in df.columns:
            condition_map = {
                ...
                18: 'Heavy Rain Shower', 19: 'Sleet Shower',
                20: 'Heavy Sleet Shower', 21: 'Snow Shower',
        ...
        # Save data
        output_file = self.data_dir / f"{start_date}_to_{end_date}_{station_name}_{state.upper()}.csv"
    ...
    def download_solar_data(
        ...
        if not config_path.exists():
            ...
            # Save config
            self._write_config(
                config_path, api_key, first_name, 
                last_name, affiliation, email
            )
        ...
        try:
            ...
            solar_df.index = pd.date_range(
                f'1/1/{year}',
                freq='60Min',
                periods=min_in_year // 60
            )
